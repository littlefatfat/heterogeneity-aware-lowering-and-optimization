...
struct _odla_computation {
  ...
  // unorderd_set count mrytodo
  std::unordered_set<odla_value> dynamic_shape_inputs;
  std::unordered_set<odla_value> dynamic_shape_outputs;
...
odla_status odla_SetComputationItem(odla_computation computation,
  ...
  switch (type) {
    ...
    case ODLA_DYNAMIC_SHAPE_INPUTS: // mry
      odla_value input;
      input = *reinterpret_cast<odla_value*>(value);
      computation->dynamic_shape_inputs.insert(input);
      break;

    case ODLA_DYNAMIC_SHAPE_OUTPUTS:
      odla_value output;
      output = *reinterpret_cast<odla_value*>(value);
      computation->dynamic_shape_outputs.insert(output);
      break;
...
odla_status odla_BindToArgument(odla_value value, const odla_void* data_ptr,
  ...
  if (g_comp && g_comp->is_dynamic_shape) {
    if (g_comp->dynamic_shape_inputs.find(value) !=
        g_comp->dynamic_shape_inputs.end()) {
...
odla_status odla_BindToOutput(odla_value value, odla_void* data_ptr,
  ...
  if (g_comp && g_comp->is_dynamic_shape) {
    if (g_comp->dynamic_shape_outputs.find(value) !=
        g_comp->dynamic_shape_outputs.end()) {
      real_shape = context->real_shapes[value];
    }
  }
  size_t bytes =
...
odla_status odla_ExecuteComputation(odla_computation comp, odla_context context,
  ...
  if (comp->is_dynamic_batch) {
    ...
    for (auto& input_ptr : context->input_ptrs) {
      ...
      if (comp->dynamic_shape_inputs.find(value) !=
          comp->dynamic_shape_inputs.end()) {
...
static const int s2[] = {1, 1, 2, 2};
static const int s4[] = {1, 1, 4, 4};
static const int s8[] = {1, 1, 8, 8};
...
odla_value odla_Resize(odla_value input, odla_interpolation_mode interpolation,
  ...
  auto shape = g_comp->network->addShape(*input)->getOutput(0);
  const int* scale = s2;
  const std::string id((const char*)value_id);
  if (id == "val_788") {
    scale = s4;
  }
  if (id == "val_799") {
  ...
  auto two = odla_CreateConstant(
      odla_value_type{ODLA_INT32, odla_value_shape{1, {4}}}, scale, nullptr);
  auto new_shape =
      g_comp->network
          ->addElementWise(*shape, *two, nvinfer1::ElementWiseOperation::kPROD)
          ->getOutput(0);
...
odla_values odla_LSTM(odla_value input, odla_rnn_weight_format weight_format,
  ...
  bool is_dynamic_seq = seq_size < 0;
  int max_seq_size = is_dynamic_seq ? 256 : seq_size;

  auto input_t = input->tensor;
  // input layout [seq, batch, input]

  // pad to [max_seq, batch, input]
  {
    std::string name((const char*)value_id.value_ids[0]);

    auto name_seq = name + "_seq";
    auto name_shape = name + "_shape";

    auto input_shape_tensor = odla_Shape(input, {.size = 1, {3}},
                                         (odla_value_id)(name_shape.c_str()));

    auto max_seq_tensor = odla_CreateConstant(
        odla_value_type{ODLA_INT32, odla_value_shape{1, {1}}}, &max_seq_size,
        nullptr);
    auto actual_seq_tensor =
        odla_Slice(input_shape_tensor, (const odla_int32[]){0},
                   (const odla_int32[]){1}, (const odla_int32[]){1},
                   {.size = 1, {1}}, (odla_value_id)name_seq.c_str());
    auto diff = odla_Sub(max_seq_tensor, actual_seq_tensor, nullptr);
    int32_t vals[] = {1, 288};
    auto bi = odla_CreateConstant(
        odla_value_type{ODLA_INT32, odla_value_shape{1, {2}}}, vals, nullptr);
    ITensor* new_shape[] = {diff->tensor, bi->tensor};
    auto padding_shape = g_comp->network->addConcatenation(new_shape, 2);
    auto fill = g_comp->network->addFill(
        nvinfer1::Dims{1, {1}}, nvinfer1::FillOperation::kRANDOM_UNIFORM);
    fill->setInput(0, *padding_shape->getOutput(0));
    auto ax = fill->getDimensions();
    std::cout << ax.nbDims;
    ITensor* new_inputs[] = {input_t, fill->getOutput(0)};
    auto cat0 = g_comp->network->addConcatenation(new_inputs, 2);
    cat0->setName("KKKKe");
    input_t = cat0->getOutput(0);
  }
  ...
  transpose_layer->setReshapeDimensions(
      nvinfer1::Dims{3, {batch_size, max_seq_size, input_size}});
  transpose_layer->setZeroIsPlaceholder(true);

  input_t = transpose_layer->getOutput(0);
  nvinfer1::IRNNv2Layer* rnn_layer =
      g_comp->network->addRNNv2(*input_t, num_layers, hidden_size, max_seq_size,
  ...
  if (is_dynamic_seq) {
    ...
    auto shape = g_comp->network->addShape(*input)->getOutput(0);
    auto actual_seq_size = odla_Slice(
        odla_Shape(input, {.size = 1, {3}}, (odla_value_id)name_shape.c_str()),
        (const odla_int32[]){0}, (const odla_int32[]){1},
        (const odla_int32[]){1}, {.size = 1, {1}},
        (odla_value_id)name_seq.c_str());
    rnn_layer->setSequenceLengths(*actual_seq_size);
  }
  ...
  // Bias shape [dir, 4 * hidden_size] or [dir, 8 *
  // hidden_size]
  ...
  // [0]: [seq, batch, dir, hidden]  --> [seq, dir, batch,
  // hidden] [1] [2] : [batch, dir, hidden]  --> [dir, batch,
  // hidden]
